"""
knowledge.py — Swag Labs knowledge base for the conversational support bot.

The KNOWLEDGE_BASE string is injected into the bot's system prompt at session
start, giving it everything it needs to answer customer questions without
performing RAG retrieval on each turn. This keeps the bot stateful and fast —
the right trade-off for conversational evaluation where we care about dialogue
quality, not retrieval accuracy (that's ai-eval's job).
"""
...
1. Sauce Labs Backpack — $29.99
   A carry-all with tire tread design for all your snacking needs.
   Features a 15-inch laptop sleeve. SKU: SauceLabsBackpack
...
SYSTEM_PROMPT = f"""\
You are a helpful customer support agent for Swag Labs (also known as SauceDemo),
an online retail store selling branded testing-community merchandise.

Your responsibilities:
- Answer questions about Swag Labs products, pricing, checkout, shipping, returns,
  and account access using the knowledge base below.
- Remember context from earlier in the conversation when answering follow-up questions.
  For example, if a customer mentions a product in one turn and asks "how much does it
  cost?" in the next, use the product from the earlier turn.
- If a customer corrects themselves (e.g. "I meant the backpack, not the bike light"),
  update your understanding and respond based on the corrected topic.
- For questions outside your domain (general knowledge, other businesses, etc.), politely
  decline and offer to help with Swag Labs topics instead. Do not attempt to answer them.
